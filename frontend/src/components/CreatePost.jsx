...
const CreatePost = ({ onCreated }) => {
  ...
  const submit = async () => {
    ...
    try {
      ...
      form.append("category", title.toLowerCase());
      ...
      if (mediaType === "image" && baseCanvasRef.current) {
        const blob = await new Promise((resolve) => {
          ...
          out.toBlob(resolve, "image/webp", 0.85);
        });
        if (blob) {
          form.append("media", new File([blob], "post.webp", { type: "image/webp" }));
        }
  ...
  const onMediaChange = async (file) => {
    if (!file) return;
    ...
    if (isImage) {
      try {
        finalFile = await compressImage(file, { maxWidth: 1200, quality: 0.8 });
      } catch (err) {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
      <div className="mb-2">
        <label htmlFor="create-post-title" className="block text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-1">
        ...
        <select
          ...
          className="w-full border dark:border-gray-700 rounded-lg px-3 py-2 focus:outline-none bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-100 font-semibold"
        >
      ...
      {mediaType === "image" && mediaUrl ? (
        <div className="mt-3">
          ...
          <div
            className="border border-gray-200 dark:border-gray-700 rounded overflow-hidden relative touch-none mt-3 max-w-md w-full bg-black mx-auto aspect-video"
      ...
      {mediaType === "video" && mediaUrl ? (
        <div className="mt-3">
          <div
            className="border border-gray-200 dark:border-gray-700 rounded overflow-hidden max-w-md w-full bg-black mx-auto aspect-video"
          >
            <video
              src={mediaUrl}
              controls
